...
class GraspAffordanceNet(nn.Module):
    def __init__(self, seed_feature_dim):
        ...
        self.conv1 = nn.Conv1d(seed_feature_dim, 256, 1)  # Adjusted input channel size to match PointNet++ output
    ...
    def forward(self, seed_features, end_points):
        ...
        # Further processing for grasp affordance
        features = F.relu(self.bn1(self.conv1(seed_features)))
        graspable_score = self.conv2(features)
        graspable_score = graspable_score.transpose(2, 1).contiguous()
        end_points['graspness_score'] = graspable_score
...
class ApproachVecNet(nn.Module):
    ...
    def forward(self, seed_features, end_points):
        ...
        if self.is_training:
            ...
            vp_rot =batch_viewpoint_params_to_matrix_data(vp_xyz_, batch_angle).view(B, num_seed, 3, 3)
