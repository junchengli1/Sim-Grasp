...
parser.add_argument('--grid_space', type=int,default=200, help='grid space for cloning')
        ...
        for i in range(num_envs):
            ...
            if i == num_envs - 1:
                ...
                if args.save_pkl_flag ==True:
                    file_path = os.path.join(args.data_path, f"stage_{stage_ind}", f"stage_{stage_ind}_grasp_simulation_candidates_test.pkl")
...
class RobotSpawner:
    ...
    def spawn(self, num_envs,  good_suctions_translation,good_suctions_rotation, envs_positions):
        for env_i in range(num_envs):
            ...
            asset_path = "/media/juncheng/Disk4T1/Sim-Grasp/Props/fetch.usd"
...
def sim_suction_simulation():
    ...
    for stage_ind in range(args.start_stage,args.end_stage):
        ...
        candidate=args.data_path+f"/stage_{stage_ind}"+ f"/stage_{stage_ind}_grasp_candidates_after_overlap_test.pkl"
        ...
        for object_number in candidates.keys():
            ...
            good_suctions_translation = [sample["grasp_translation"] 
                 for sample in candidates[object_number]["grasp_samples"] 
                 if sample["collision_quality"] == 1]
            good_suctions_rotation = [sample["grasp_rotation_matrix"] 
                 for sample in candidates[object_number]["grasp_samples"] 
                 if sample["collision_quality"] == 1]
