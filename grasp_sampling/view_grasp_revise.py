...
parser.add_argument("--stage_ID", type=int, default=148, help='stage ID number [default: 0]')
...
def draw(DATA_ROOT,STAGE_ID,MODE,GROUND_PLANE,GRIPPER,SYMBOL):
    ...
    if GRIPPER=="Fetch":
        candidate_overlap_path=DATA_ROOT+f"stage_{STAGE_ID}"+ f"/stage_{STAGE_ID}_grasp_candidates_after_overlap_test.pkl"
    ...
    if MODE=="collision":
        ...
        for object_index in candidate_overlap.keys():
            ...
            for i in range(len(grasp_samples)):
                ...
                suction_translation=grasp_samples[i]["grasp_translation"]
                suction_rotation_matrix=grasp_samples[i]["grasp_rotation_matrix"]
                collision=True
                T = np.eye(4)
    ...
    if MODE=="simulation":
        ...
        for object_index in candidate_simulation.keys():
            ...
            for i in range(len(good_candidates)):
                ...
                suction_translation=good_candidates[i]["grasp_translation"]
                suction_rotation_matrix=good_candidates[i]["grasp_rotation_matrix"]
                collision=True
                T = np.eye(4)
                T[:3, :3] = suction_rotation_matrix
                T[0, 3] = suction_translation[0]
             ...
             # Show only up to 500 bad candidates
            max_bad_candidates_to_show = 50
            for i in range(min(len(bad_candidates), max_bad_candidates_to_show)):
                suction_translation = bad_candidates[i]["grasp_translation"]
                suction_rotation_matrix = bad_candidates[i]["grasp_rotation_matrix"]
                collision = True
                T = np.eye(4)
                T[:3, :3] = suction_rotation_matrix
                T[0, 3] = suction_translation[0]
