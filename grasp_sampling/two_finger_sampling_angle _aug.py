...
parser.add_argument('--pcl_path', type=str, default=(base_dir.parent / "pointcloud_train_grasp").as_posix(), help='point cloud path')
parser.add_argument('--data_path', type=str, default=(base_dir.parent / "synthetic_data_grasp_test").as_posix(), help='data path')
...
parser.add_argument('--save_pcd_flag', type=bool, default=True, help='Save each objects as pcd file')
...
def calculate_rotation_matrix(normal_vector):
    Z = np.array([0, 0, 1])
    N = normal_vector / np.linalg.norm(normal_vector)  # Normalize N
    R = np.cross(N, Z)
    theta = np.arccos(np.dot(N, Z))
    
    # Construct skew-symmetric matrix for R
...
def construct_frames_from_normals(normal_vectors):
    # Use the normal vectors as the z-axis
    axis_z = normal_vectors
    # Create arrays of ones and zeros for constructing the x-axis and y-axis
    ones = np.ones(axis_z.shape[0])
    zeros = np.zeros(axis_z.shape[0])
    # Construct the y-axis as a cross product of the z-axis and an arbitrary vector
    axis_y = np.cross(axis_z, np.stack([zeros, zeros, ones], axis=-1))
    ...
    rotation_matrices = np.stack([axis_x, axis_y, axis_z], axis=-1)
...
def sample_pinch_grasp (rotation_angle,suction_translation,suction_rotation_matrix,draw,the_world,visualize_flag):
            ...
            R1 = np.array([[cos_angle, -sin_angle, 0],
            ...
            R2=suction_rotation_matrix
            #print("R2:",R2.shape)
            suction_rotation_matrix_new = np.matmul(R2.reshape(3,3), R1.reshape(3,3))
    ...
    def __getitem__(self, stage_ind,save_pcl_flag,save_seal_flag):
        ...
        for prim in Usd.PrimRange(curr_prim):
            if prim.IsA(UsdGeom.Xform):
                ...
                collisionAPI.CreateCollisionEnabledAttr(False)
        ...
        for obj_num in all_instance_seg_ids:
            ...
            if obj_num!=0:
                ...
                # Check if the point cloud has only one point
                if torch_pts.shape[1] == 1:
                    print("Point cloud has only one point. Skipping sampling.")
                    point_idx = torch.tensor([0])  # Assuming you need to return the index of the single point

                    #continue                    
                    # Handle the single-point case appropriately here
                    # For example, you might directly use this point or skip processing this point cloud
                else:
                    # If the point cloud has more than one point, proceed with sampling
                    if torch_pts.shape[1] < 300:
                        point_idx = farthest_point_sampler(torch_pts, torch_pts.shape[1])
                    else:
                        point_idx = farthest_point_sampler(torch_pts, 300)
                ...
                # Ensure FPS_index is an array with a shape that np.random.shuffle can work with
                if not isinstance(FPS_index, np.ndarray) or FPS_index.shape == ():
                    # If FPS_index is a scalar or unsized, convert it to a 1D array with one element
                    FPS_index = np.array([FPS_index])

                # Now FPS_index is guaranteed to be a numpy array with a proper shape
                ...
                for index in FPS_index:
                    ...
                    #print(index)
                    #t_ori, t_translate, _,normal_vector,normal_vector_opposite=compute_darboux_frame(points, index, kdtree, normals)
                    # Sample approach directions within a cone around the normal vector
                        # Extract the point corresponding to the current index
                    t_ori = points[index,:]

                   
                    normal_vector = normals[index,:] # Extract the normal vector
                    t_translate= t_ori+normal_vector*1.5
                  

                    sampled_approach_directions = sample_cone(normal_vector, 3, 8)
                    #R = calculate_rotation_matrix(normal_vector)
                    rotation_matrices_normals = construct_frames_from_normals(sampled_approach_directions)

                    #print(rotation_matrices_normals.shape)
                    ...
                    if collision_flag== False:
                        ...
                        for sampled_rotations_j in range(len(rotation_matrices_normals)): 
                            for pinch_grasp_samples_j in range(pinch_grasp_samples):
                                for standoff_j in standoff_depths:
                        # Compute the rotation angle in radians
                                    rotation_angle =  np.pi * pinch_grasp_samples_j/ pinch_grasp_samples  # This will range from 0 to 2*pi
                                
                                    offset_translation = t_ori - standoff_j * sampled_approach_directions[sampled_rotations_j]  # Note the minus sign


                                    grasp_translation, grasp_rot, collision_quality=sample_pinch_grasp(rotation_angle,offset_translation,rotation_matrices_normals[sampled_rotations_j],draw,the_world,False)                                     
                                    ...
                                    candidates[obj_num]["grasp_samples"].append({
                                        ...
                                        "grasp_rotation_matrix": grasp_rot,
                                        "stand_off":standoff_j,
                                        "rotation_angle":pinch_grasp_samples_j,
                ...
                if save_seal_flag == True:
                    ...
                    #candidates[obj_num]["grasp_samples"] = good_candidates + bad_candidates


                    file_path = os.path.join(self.data_root, f"stage_{stage_ind}", f"stage_{stage_ind}_grasp_candidates_after_overlap_test.pkl")
