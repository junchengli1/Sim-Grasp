...
class Dataset():
    ...
    def __getitem__(self, stage_ind):
        ...
        #with open('/home/juncheng/Downloads'+"/"+"seg_dic"+".pkl", "rb") as f:
                    #pickle.dump(seg_map, f)
                #seg_dic=pickle.load(f)
        #f.close()
        with open("/media/juncheng/jason-msral/"+"seg_dic.pkl", "rb") as f:
                    #pickle.dump(seg_map, f)
                seg_dic=pickle.load(f)
        f.close()
        ...
        hand_mesh.paint_uniform_color([0, 0.7, 0])
        #hand_mesh=o3d.io.read_point_cloud("/home/juncheng/Documents/symbol.ply")
        ...
        asset=create_prim(
                                    ...
                                    #orientation=euler_angles_to_quat(Rotation.random().as_euler('zyx', degrees=True))
            ...
            if i!=0:
               ...
               #pcd.estimate_normals(search_param=o3d.geometry.KDTreeSearchParamRadius(radius=0.5),fast_normal_computation=False)
                ...
                if len(normals)!= len(points):
                    pcd.estimate_normals(search_param=o3d.geometry.KDTreeSearchParamHybrid(radius=1.5, max_nn=500))
                ...
                for index in index_list:
                       ...
                       # while not setup_task.done():
                        ...
                        if (check_raycast(t2,normal_vector))==None or check_raycast(t2,normal_vector)[0]==f"/World/objects/object_{i-1}":
        
                            approach_sampler=SurfaceApproachSampler(standoff_depth, 30, 0.0, mesh_normal,gripper_normal,t)
                            
                            result=approach_sampler.sample(5)
                            for grasp_num in range(len(result['poses'])):
                                ...
                                xform_prim.set_world_pose(position =result['poses'][grasp_num][0:3],orientation=quaternion_wxyz)   
                                
            
                                simulation_app.update()   
                            
                                #overlap=overlap_check([1,1,1],[T4[0, 3],T4[1, 3],T4[2, 3]],[0,0,1,0])
                                base_path=PhysicsSchemaTools.encodeSdfPath(Sdf.Path("/World/hand/hand/base"))
                                f_left_path=PhysicsSchemaTools.encodeSdfPath(Sdf.Path("/World/hand/hand/left"))
                                f_right_path=PhysicsSchemaTools.encodeSdfPath(Sdf.Path("/World/hand/hand/right"))

                    
                                check1=get_physx_scene_query_interface().overlap_mesh_any(base_path[0],base_path[1])
                                check2=get_physx_scene_query_interface().overlap_mesh_any(f_left_path[0],f_left_path[1])
                                check3=get_physx_scene_query_interface().overlap_mesh_any(f_right_path[0],f_right_path[1])
                                quaternion_xyzw=[quaternion_wxyz[1],quaternion_wxyz[2],quaternion_wxyz[3],quaternion_wxyz[0]]
                                
                                
                                box_base_T = np.eye(4)
                                #T2[:3, :3] = R
                                box_base_T[0:3, 3] = [2.5,0,0]
                               ...
                               #origin[2, 3] = suction_prim[2]
                                ...
                                overlap_api=overlap_check([2,3,1],box_base_T[0:3, 3],quaternion_xyzw)
                                check4=overlap_api.collision_check()
                                ...
                                if check1==False and check2==False and check3==False and check4==True:
                                    ...
                                    rotation_after_overlap_pass.append(gripper_orientation)
                                    translation_after_overlap_pass.append([result['poses'][grasp_num][0],result['poses'][grasp_num][1],result['poses'][grasp_num][2]])
                                else:
                                    translation_after_overlap_fail.append([result['poses'][grasp_num][0],result['poses'][grasp_num][1],result['poses'][grasp_num][2]])
                                    rotation_after_overlap_fail.append(gripper_orientation)
                ...
                candidates[i]=dict()
                candidates[i]["segmentation_id"]=seg_dic[info[i-1][3]]
                candidates[i]["object_name"]=info[i-1][3]
                candidates[i]["rotation_after_overlap_pass"]=rotation_after_overlap_pass
                candidates[i]["translation_after_overlap_pass"]=translation_after_overlap_pass
                candidates[i]["rotation_after_overlap_fail"]=rotation_after_overlap_fail
                candidates[i]["translation_after_overlap_fail"]=translation_after_overlap_fail
                
                candidates[i]["total_candidates"] = len(translation_after_overlap_pass)+len(translation_after_overlap_fail)
                candidates[i]["total_candidates_pass_overlap"] = len(translation_after_overlap_pass)
                #candidates[i]["total_candidates_pass_seal"] = len(translation_after_seal_pass)
        ...
        #with open(self.data_root+f"/stage_{stage_ind}/"+f"stage_{stage_ind}_grasp_candidates_after_overlap"+".pkl", 'rb') as f:
...
#data_root="/home/juncheng/Downloads/revision"
...
for i in range(2,len(stage_root)):
    #print(i)
