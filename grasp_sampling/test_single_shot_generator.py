...
parser.add_argument('--dataset_type', type=str, default='test_similar', choices=['test_similar', 'test_novel'], help='Choose dataset type: similar or novel')
...
args.save_pcl_path = (base_dir.parent / "single_pointcloud_closer").as_posix()
...
class single_shot_rep():
    ...
    def new_positions_spherical_coordinates(self):
        radius = np.random.uniform(100.0,400.0, (10200,1)) 
        ...
        y=ar
        return (x,y,z)
    def new_positions_spherical_coordinates2(self):
        radius = np.random.uniform(100.0, 400.0, (10200, 1))
        theta = np.random.uniform(-math.pi / 6, math.pi / 6, (10200, 1))  # 60 degrees field of view
        phi = np.arccos(1 - 2 * np.random.uniform(0, 1, (10200, 1)))  # Uniform distribution over the sphere

        x = radius * np.sin(phi) * np.cos(theta)
        y = radius * np.sin(phi) * np.sin(theta)
        z = radius * np.cos(phi)  # Position the camera above the scene
        ar = np.array(y)+50
        y=ar
        return (x, y, z)
        ...
        with rep.new_layer():
            ...
            for frame in range(args.frame_per_stage):
                ...
                if args.save_pcl_flag:
                    np.savez_compressed(args.save_pcl_path+f"/{index}_{frame}.npz",point_clouds_stage)
